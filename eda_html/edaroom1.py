...
df_room1 = pd.read_csv('C:/Users/mgssr/Desktop/vscode/mane_room_1.csv')
df_room2 = pd.read_csv('C:/Users/mgssr/Desktop/vscode/mane_room_2.csv')
df_room1_2022 = pd.read_csv('C:/Users/mgssr/Desktop/vscode/mane_room_1_2022.csv')
df_room1_2023 = pd.read_csv('C:/Users/mgssr/Desktop/vscode/mane_room_1_2023.csv')
df_room1_2024 = pd.read_csv('C:/Users/mgssr/Desktop/vscode/mane_room_1_2024.csv')
...
df_room1_2022 = df_room1_2022.drop(columns=columns_to_drop)
df_room1_2023 = df_room1_2023.drop(columns=columns_to_drop)
df_room1_2024 = df_room1_2024.drop(columns=columns_to_drop)
df_room2_2022 = df_room2_2022.drop(columns=columns_to_drop)
df_room2_2023 = df_room2_2023.drop(columns=columns_to_drop)
df_room2_2024 = df_room2_2024.drop(columns=columns_to_drop)

# Data cleaning and transformation for df_room1
df_room1['time_stamp'] = pd.to_datetime(df_room1['time_stamp'], dayfirst=True)
numeric_columns = ['co2', 'humidity', 'temperature', 'voc', 'pressure']
df_room1[numeric_columns] = df_room1[numeric_columns].apply(pd.to_numeric, errors='coerce')

# Create directory for saving images
os.makedirs('images', exist_ok=True)

# Hourly, weekday, and monthly statistics
hourly_mean = df_room1.groupby(df_room1['time_stamp'].dt.hour).mean().rename_axis('hour').reset_index()
hourly_median = df_room1.groupby(df_room1['time_stamp'].dt.hour).median().rename_axis('hour').reset_index()
weekday_mean = df_room1.groupby(df_room1['time_stamp'].dt.weekday).mean().rename_axis('weekday').reset_index()
weekday_median = df_room1.groupby(df_room1['time_stamp'].dt.weekday).median().rename_axis('weekday').reset_index()
monthly_mean = df_room1.groupby(df_room1['time_stamp'].dt.month).mean().rename_axis('month').reset_index()
monthly_median = df_room1.groupby(df_room1['time_stamp'].dt.month).median().rename_axis('month').reset_index()
...
plt.tight_layout()
plt.savefig('images/room1_variation_plots.png')
plt.close(fig)

# Correlation analysis
correlations = df_room1[numeric_columns].corr()
...
df_room1['year'] = df_room1['time_stamp'].dt.year
df_room1['month'] = df_room1['time_stamp'].dt.month
df_room1['day'] = df_room1['time_stamp'].dt.day
...
for variable in variables:
    for year in years:
        year_data = df_room1[df_room1['year'] == year]
        ...
        plt.title(f'{variable.capitalize()} Calendar Chart for Mane_Room_1 - {year}')
        plt.xlabel('Day')
        plt.ylabel('Month')
        plt.tight_layout()
...
df_room1['date'] = df_room1['time_stamp'].dt.date
df_room1['hour'] = df_room1['time_stamp'].dt.hour

# Group by date and hour and count the number of entries
hourly_entry_counts = df_room1.groupby(['date', 'hour']).size().reset_index(name='count')
...
<head>
    ...
    <title>Room 1 Variation Plots</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Room 1 Variation Plots</h1>
        <div class="plot">
            <h2>Hourly, Weekday, and Monthly Statistics</h2>
            <img src="images/room1_variation_plots.png" alt="Room 1 Variation Plots">
        </div>
        <div class="plot">
...
with open('room1_variation_plots.html', 'w') as f:
    f.write(html_content)

# Open the HTML file in the default browser
webbrowser.open(f'file://{os.path.realpath("room1_variation_plots.html")}')
print("CSV Data Shape:", df_room1.shape)
