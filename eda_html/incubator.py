...
data = pd.read_csv('C:/Users/mgssr/Desktop/vscode/incubator.csv')

# Data cleaning and transformation for the dataset
data['time_stamp'] = pd.to_datetime(data['time_stamp'], format='%d/%m/%y %H:%M')
numeric_columns = ['inc_co2', 'inc_temp']
data[numeric_columns] = data[numeric_columns].apply(pd.to_numeric, errors='coerce')

# Create directory for saving images
image_dir = 'imagesincubator'
os.makedirs(image_dir, exist_ok=True)

# Hourly, weekday, and monthly statistics
hourly_mean = data.groupby(data['time_stamp'].dt.hour).mean().rename_axis('hour').reset_index()
hourly_median = data.groupby(data['time_stamp'].dt.hour).median().rename_axis('hour').reset_index()
weekday_mean = data.groupby(data['time_stamp'].dt.weekday).mean().rename_axis('weekday').reset_index()
weekday_median = data.groupby(data['time_stamp'].dt.weekday).median().rename_axis('weekday').reset_index()
monthly_mean = data.groupby(data['time_stamp'].dt.month).mean().rename_axis('month').reset_index()
monthly_median = data.groupby(data['time_stamp'].dt.month).median().rename_axis('month').reset_index()

# Plot hourly, weekday, and monthly statistics
fig, axs = plt.subplots(4, 2, figsize=(15, 20))

# Hourly plots
axs[0, 0].plot(hourly_mean['hour'], hourly_mean['inc_temp'], label='Mean Temperature')
axs[0, 0].plot(hourly_median['hour'], hourly_median['inc_temp'], label='Median Temperature')
axs[0, 0].set_title('Temperature Variation by Hour')
axs[0, 0].legend()

axs[0, 1].plot(hourly_mean['hour'], hourly_mean['inc_co2'], label='Mean CO2')
axs[0, 1].plot(hourly_median['hour'], hourly_median['inc_co2'], label='Median CO2')
axs[0, 1].set_title('CO2 Variation by Hour')
axs[0, 1].legend()

# Weekday plots
axs[1, 0].plot(weekday_mean['weekday'], weekday_mean['inc_temp'], label='Mean Temperature')
axs[1, 0].plot(weekday_median['weekday'], weekday_median['inc_temp'], label='Median Temperature')
axs[1, 0].set_title('Temperature Variation by Weekday')
axs[1, 0].legend()

axs[1, 1].plot(weekday_mean['weekday'], weekday_mean['inc_co2'], label='Mean CO2')
axs[1, 1].plot(weekday_median['weekday'], weekday_median['inc_co2'], label='Median CO2')
axs[1, 1].set_title('CO2 Variation by Weekday')
axs[1, 1].legend()

# Monthly plots
axs[2, 0].plot(monthly_mean['month'], monthly_mean['inc_temp'], label='Mean Temperature')
axs[2, 0].plot(monthly_median['month'], monthly_median['inc_temp'], label='Median Temperature')
axs[2, 0].set_title('Temperature Variation by Month')
axs[2, 0].legend()

axs[2, 1].plot(monthly_mean['month'], monthly_mean['inc_co2'], label='Mean CO2')
axs[2, 1].plot(monthly_median['month'], monthly_median['inc_co2'], label='Median CO2')
axs[2, 1].set_title('CO2 Variation by Month')
axs[2, 1].legend()
...
for variable in variables:
    for year in years:
        ...
        plt.title(f'{variable.capitalize()} Calendar Chart for {year}')
        plt.xlabel('Day')
        plt.ylabel('Month')
        plt.tight_layout()
        plt.savefig(f'{image_dir}/{variable}_calendar_chart_{year}.png')
        plt.close()
