...
# Load data from freezer.csv
freezer_df = pd.read_csv('C:/Users/mgssr/Desktop/vscode/freezer.csv')

# Data cleaning and transformation for freezer_df
freezer_df['time_stamp'] = pd.to_datetime(freezer_df['time_stamp'], format='%d/%m/%y %H:%M')
numeric_columns = ['temperature']
freezer_df[numeric_columns] = freezer_df[numeric_columns].apply(pd.to_numeric, errors='coerce')

# Create directory for saving images
os.makedirs('imagesfreezer', exist_ok=True)

# Hourly, weekday, and monthly statistics
hourly_mean = freezer_df.groupby(freezer_df['time_stamp'].dt.hour).mean().rename_axis('hour').reset_index()
hourly_median = freezer_df.groupby(freezer_df['time_stamp'].dt.hour).median().rename_axis('hour').reset_index()
weekday_mean = freezer_df.groupby(freezer_df['time_stamp'].dt.weekday).mean().rename_axis('weekday').reset_index()
weekday_median = freezer_df.groupby(freezer_df['time_stamp'].dt.weekday).median().rename_axis('weekday').reset_index()
monthly_mean = freezer_df.groupby(freezer_df['time_stamp'].dt.month).mean().rename_axis('month').reset_index()
monthly_median = freezer_df.groupby(freezer_df['time_stamp'].dt.month).median().rename_axis('month').reset_index()

# Plot hourly, weekday, and monthly statistics
fig, axs = plt.subplots(4, 2, figsize=(15, 20))

# Hourly plots
axs[0, 0].plot(hourly_mean['hour'], hourly_mean['temperature'], label='Mean Temperature')
axs[0, 0].plot(hourly_median['hour'], hourly_median['temperature'], label='Median Temperature')
axs[0, 0].set_title('Temperature Variation by Hour')
axs[0, 0].legend()

# Weekday plots
axs[1, 0].plot(weekday_mean['weekday'], weekday_mean['temperature'], label='Mean Temperature')
axs[1, 0].plot(weekday_median['weekday'], weekday_median['temperature'], label='Median Temperature')
axs[1, 0].set_title('Temperature Variation by Weekday')
axs[1, 0].legend()

# Monthly plots
axs[2, 0].plot(monthly_mean['month'], monthly_mean['temperature'], label='Mean Temperature')
axs[2, 0].plot(monthly_median['month'], monthly_median['temperature'], label='Median Temperature')
axs[2, 0].set_title('Temperature Variation by Month')
axs[2, 0].legend()
...
correlations = freezer_df[numeric_columns].corr()
...
freezer_df['year'] = freezer_df['time_stamp'].dt.year
freezer_df['month'] = freezer_df['time_stamp'].dt.month
freezer_df['day'] = freezer_df['time_stamp'].dt.day
variables = ['temperature']
years = freezer_df['year'].unique()

month_names = {1: 'January', 2: 'February', 3: 'March', 4: 'April', 5: 'May', 6: 'June', 7: 'July', 8: 'August', 9: 'September', 10: 'October', 11: 'November', 12: 'December'}

for variable in variables:
    for year in years:
        year_data = freezer_df[freezer_df['year'] == year]
...
daily_entry_counts = freezer_df.groupby(['year', 'month', 'day']).size().reset_index(name='count')
...
for year in years:
    df_year = freezer_df[freezer_df['year'] == year]
